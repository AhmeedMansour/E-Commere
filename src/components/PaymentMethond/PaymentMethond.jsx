...
export default function PaymentMethond() {
  ...
  async function visaOrder(values) {
    try {
      ...
      const { data } = await axios.post(`https://ecommerce.routemisr.com/api/v1/orders/checkout-session/${cartId}?url=${window.location.origin}`,values,{headers:{token:localStorage.getItem("token")}})
      window.open(data.session.url,"_blank")
      if (data.status === "success") {
        toast.success("Order placed successfully!", { autoClose: 3000 });}
    }
    catch (err) {
      console.log(err, "Visa ERRRORR!");
      toast.error("Cart is Empty!");
    }
      ...
      <div className="bg-white dark:bg-gray-900 shadow-lg rounded-xl p-6">
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-5">
          <input
            ...
            className="w-full px-4 py-3 border rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500"
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              type="submit"
              className="w-full  cursor-pointer bg-green-600 hover:bg-green-700 text-white font-medium py-3 rounded-lg transition-all"
            >
              Cash on Delivery
            </button>
            <button
              ...
              type="submit"
              className="w-full cursor-pointer  bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 rounded-lg transition-all"
            >
              Pay with Visa
            </button>
          </div>
